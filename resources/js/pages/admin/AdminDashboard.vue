<template>
  <div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
      ...
      >
        <div class="w-10 h-10 flex items-center justify-center rounded-full bg-blue-100 text-blue-600 text-2xl">
          <i :class="item.icon"></i>
        </div>
...
const stats = ref({
  pegawai: { label: "Pegawai", count: 0, icon: "fas fa-user-tie", route: "/admin/pegawai" },
  user: { label: "User", count: 0, icon: "fas fa-users", route: "/admin/user" },
  spt: { label: "SPT", count: 0, icon: "fas fa-file-signature", route: "/admin/spt" },
  sppd: { label: "SPPD", count: 0, icon: "fas fa-file-alt", route: "/admin/sppd" },
});
...
onMounted(async () => {
  try {
    ...
    stats.value.user.count = res.data.user;
